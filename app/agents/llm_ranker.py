...
from app.schemas import NormalizedOffer, RankingMode
...
- Use Rs. for rupees. Be conversational. Max 80 words. No bullet points.
"""
...
async def llm_generate_explanation(
    ...
    for o in ranked_offers[:5]:
        p = f"Rs.{o.effective_price:,.0f}" if o.effective_price else "N/A"
        ...
        lines.append(
            f"- {o.platform_name}: {p} | del:{d} | "
            f"rating:{r}/5 | match:{int(o.match_score*100)}%{badge}"
        )

    top_price = f"Rs.{top.effective_price:,.0f}" if top.effective_price else "price unknown"
    user = (
        f"Product: {product_query}\nPreference: {mode.value}\n\n"
        f"Ranked offers:\n" + "\n".join(lines) +
        f"\n\nRecommend {top.platform_name} at {top_price}."
    )
